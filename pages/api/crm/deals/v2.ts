...
async function fetchAllDeals(req: NextApiRequest) {
  const { clinic_id: clinicId, auth_token: authToken } = cookie.parse(
    req.headers.cookie,
  );
  const queryRecord: Record<string, string> = req.query as Record<
    string,
    string
  >;
  const queryString = new URLSearchParams(queryRecord).toString();
  return axios.get(`${updatedServerUrl()}/crm/deals/v2?${queryString}`, {
    headers: {
