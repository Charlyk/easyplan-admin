...
const CSVImportModal = ({
  ...
  const btnTitle = useMemo(() => {
    ...
    return importBtnTitle.replace('{1}', `${rowsCount - 1}`); // subtract one row for the title row
  ...
  const isFormValid = useMemo(() => {
    const requiredFields = fields
      ...
      .map((item) => item.id)
      .sort();
    const selectedRequired = mappedFields
      .filter((item) => item.required)
      .map((item) => item.id)
      .sort();
  ...
  const pendingFields = useMemo(() => {
    ...
    const selectedRequired = mappedFields
      ...
      .map((item) => item.id)
      .sort();
    const difference = requiredFields.filter(
      (item) => !selectedRequired.includes(item),
    );
  ...
  const handleOnDrop = async (data, file) => {
    ...
    if (rowsCount - 1 > maxAllowedRows) {
      localDispatch(
        setSnackbar({
          show: true,
          message: textForKey('csv_max_rows_exceeded'),
        }),
      );
      return;
    }
  ...
  const handleUploadFile = () => {
    if (!isFormValid) {
      localDispatch(
        setSnackbar({
          show: true,
          message: textForKey('map_all_fields').replace(
            '{1}',
            pendingFields.join(', '),
          ),
        }),
  ...
  const handleFieldSelected = (fieldId, index) => {
    ...
    if (fieldId === 'none') {
      localDispatch(
        setMappedFields(mappedFields.filter((item) => item.index !== index)),
      );
      return;
    }
            ...
            <Table className={styles.table}>
              <TableHead className={styles.tableHead}>
                {data.slice(0, 1).map((item, index) => (
                  <TableRow
                    key={`header-row-${item.data.join(';')}-${index}`}
                ...
                {data.slice(0, 1).map((row, rowIndex) => (
                  <TableRow
                    key={`select-row-${row.data.join(';')}-${rowIndex}`}
                    className={styles.tableRow}
                  >
                    {row.data.map((cell, cellIndex) => (
                      <TableCell
                        key={`select-cell-${cell}-${cellIndex}`}
                      ...
                      >
                        <div className={styles.dataContainer}>
                          <Select
                            ...
                            onChange={(event) =>
                              handleFieldSelected(event.target.value, cellIndex)
                            }
                          >
                            <MenuItem
                              value='none'
              ...
              <TableBody className={styles.tableBody}>
                {data.slice(1, 10).map((item, index) => (
                  <TableRow
                    key={`value-row-${item.data.join(';')}-${index}`}
                    className={styles.tableRow}
                  >
                    {item.data.map((value, index) => (
                      <TableCell
