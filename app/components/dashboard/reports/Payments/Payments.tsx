...
const Payments: React.FC<PaymentsProps> = ({ query }) => {
  ...
  const handleFilterSubmit = async () => {
    ...
    await router.replace(
      `/reports/payments?page=0&startDate=${startDate}&endDate=${endDate}&itemsPerPage=${request.itemsPerPage}`,
    );
  };
  ...
  const handleChangePage = async (event, newPage) => {
    ...
    const endDate = moment(dateRange[1]).format('YYYY-MM-DD');
    await router.replace(
      `/reports/payments?page=${newPage}&startDate=${startDate}&endDate=${endDate}&itemsPerPage=${request.itemsPerPage}`,
    );
  };
  ...
  const handleChangeRowsPerPage = async (event) => {
    ...
    const endDate = moment(dateRange[1]).format('YYYY-MM-DD');
    await router.replace(
      `/reports/payments?page=0&startDate=${startDate}&endDate=${endDate}&itemsPerPage=${rows}`,
    );
  };
